...
class TestKrige(unittest.TestCase):
    ...
    def test_simple(self):
        for Model in self.cov_models:
            ...
            field_1 = simple.unstructured(self.pos[0])
            field_2 = simple.structured(self.pos[0])
            for i, val in enumerate(self.cond_val):
                self.assertAlmostEqual(val, field_1.values[i], places=2)
                self.assertAlmostEqual(val, field_2.values[(i,)], places=2)
            self.assertAlmostEqual(self.mean, field_1.values[-1], places=2)
            self.assertAlmostEqual(self.mean, field_2.values[(-1,)], places=2)
            ...
            for dim in self.dims[1:]:
                ...
                field_1 = simple.unstructured(self.pos[:dim])
                field_2 = simple.structured(self.pos[:dim])
                for i, val in enumerate(self.cond_val):
                    self.assertAlmostEqual(val, field_1.values[i], places=2)
                    self.assertAlmostEqual(val, field_2.values[dim * (i,)], places=2)
                self.assertAlmostEqual(self.mean, field_1.values[-1], places=2)
                self.assertAlmostEqual(
                    self.mean, field_2.values[dim * (-1,)], places=2
                )
    ...
    def test_ordinary(self):
        for Model in self.cov_models:
            ...
            field_1 = ordinary.unstructured(self.pos[0])
            field_2 = ordinary.structured(self.pos[0])
            for i, val in enumerate(self.cond_val):
                self.assertAlmostEqual(val, field_1.values[i], places=2)
                self.assertAlmostEqual(val, field_2.values[(i,)], places=2)
            ...
            for dim in self.dims[1:]:
                ...
                field_1 = ordinary.unstructured(self.pos[:dim])
                field_2 = ordinary.structured(self.pos[:dim])
                for i, val in enumerate(self.cond_val):
                    self.assertAlmostEqual(val, field_1.values[i], places=2)
                    self.assertAlmostEqual(val, field_2.values[dim * (i,)], places=2)
