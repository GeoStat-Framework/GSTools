...
class RNG(object):
    ...
    def _prepare_random_numbers(self, mode_no):
        ...
        for d in range(self.dim):
            ...
            rng = self._get_random_stream()
            iid[d] = rng.uniform(0.0, 1.0, mode_no)
        return k, iid
    ...
    def gau(self, len_scale, mode_no=1000, **kwargs):
        ...
        if self.dim == 1:
            k = self._create_empty_k(mode_no)
            rng = self._get_random_stream()
            k[0] = rng.normal(0.0, np.pi / 2.0 / len_scale ** 2, mode_no)
        elif self.dim == 2:
            ...
            def cdf(r):
                res = sps.erf(
                    r * len_scale / np.sqrt(np.pi)
                ) - 2 * r * len_scale / np.pi * np.exp(
                    -(r * len_scale) ** 2 / np.pi
                )
                return res
        ...
        def spec_dens(r):
            ...
            return (
                ...
                * np.pi ** (-self.dim / 2.0)
                / r ** self.dim
                * sub1 ** kappa
                / (1 + sub1) ** (kappa + self.dim / 2.0)
                * sps.gamma(self.dim / 2.0)
                / sps.beta(kappa, self.dim / 2.0)
            )
    ...
    def sph(self, len_scale, mode_no=1000, **kwargs):
        ...
        def cov_norm(r):
            ...
            res = (
                1.0
                - 9.0 / 16.0 * r / len_scale
                + 27.0 / 1024.0 * (r / len_scale) ** 3
            )
            res[r > 8.0 / 3.0 * len_scale] = 0.0
            return res
