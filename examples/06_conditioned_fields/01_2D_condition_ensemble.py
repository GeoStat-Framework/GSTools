...
vmax = np.max([cond_srf[f"f{i}"] for i in range(ens_no)])
sc_kwargs = dict(c=cond_val, edgecolors="k", vmin=0, vmax=vmax)
im_kwargs = dict(extent=2 * [0, 5], origin="lower", vmin=0, vmax=vmax)
for i in range(ens_no):
    # conditioned fields and conditions
    ax[i + 1, 0].imshow(cond_srf[f"f{i}"].T, **im_kwargs)
    ax[i + 1, 0].scatter(*cond_pos, **sc_kwargs)
    ax[i + 1, 0].set_ylabel(f"Field {i+1}", fontsize=10)
    ax[0, i + 1].imshow(cond_srf[f"f{i}"].T, **im_kwargs)
    ax[0, i + 1].scatter(*cond_pos, **sc_kwargs)
    ax[0, i + 1].set_title(f"Field {i+1}", fontsize=10)
    # absolute differences
    for j in range(ens_no):
        ax[i + 1, j + 1].imshow(
            np.abs(cond_srf[f"f{i}"] - cond_srf[f"f{j}"]).T, **im_kwargs
        )
